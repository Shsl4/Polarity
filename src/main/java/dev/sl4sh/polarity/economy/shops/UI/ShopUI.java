...
public class ShopUI {
    ...
    public void makeFromShopProfile(Player player, ShopProfile profile, ICustomNpc NPC){
    ...
    public void onItemLeftClick(ClickInventoryEvent.Primary event){
        ...
        if(event.getCursorTransaction().getDefault().get(Keys.ITEM_LORE).isPresent()){
            ...
            Optional<ShopRecipe> optRecipe = shopProfile.getRecipeBySnapshot(event.getCursorTransaction().getDefault());
    ...
    public static void makeTransaction(Player player, ShopRecipe recipe){
