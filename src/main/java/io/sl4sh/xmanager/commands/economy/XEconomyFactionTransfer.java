...
public class XEconomyFactionTransfer implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        ...
        if(src instanceof Player){
            ...
            transferToFaction(caller, (BigDecimal)args.getOne("amount").get(), (String)args.getOne("factionName").get());
    ...
    private void transferToFaction(Player caller, BigDecimal amount, String factionName){

        Optional<XFaction> optTargetFaction = XUtilities.getFactionByName(factionName);

        if(!optTargetFaction.isPresent()) { caller.sendMessage(Text.of(XError.XERROR_XFNULL.getDesc(), TextColors.RED, " Type /factions list to list existing factions.")); return; }
        ...
        if(!economyService.getOrCreateAccount(caller.getUniqueId()).isPresent() || !economyService.getOrCreateAccount(targetFaction.getFactionName()).isPresent()) { caller.sendMessage(Text.of(TextColors.RED, "[Economy] | Unable to access accounts. Please try again later.")); return; }
