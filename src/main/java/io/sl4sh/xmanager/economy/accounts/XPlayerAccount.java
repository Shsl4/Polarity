        ...
        if(currency instanceof XDollar && getTargetPlayer().isPresent()){
            ...
            if(cause.containsType(XFaction.class)){
                ...
                if(cause.first(XFaction.class).isPresent()){
                    ...
                    targetPlayer.sendMessage(Text.of(TextColors.AQUA, "[Economy] | You just received ", dollarCurrency.format(amount, 2), TextColors.AQUA, " from ", senderFaction.getFactionDisplayName(), TextColors.AQUA, " faction!"));
                    return;
